...
(define (eval exp env)
  (cond ((self-evaluating? exp) exp)
        ...
        ((application? exp)
         (let ((optr (actual-value (operator exp) env)))
           (if (and (pair? optr) (compound-procedure? (car optr)))
               (applye (car optr)
                       (map (λ(a b) (cons a b)) (operands exp) (cdr optr))
                       env)
               (applye optr
                       (map (λ(a) (cons a 'applicative)) (operands exp))
                       env))))
        (else
...
(define (list-of-arg-values args env)
  (map (λ(x) (delay-or-eval x env)) args))

(define (delay-or-eval x env)
  (let ((tag (cdr x)))
    (cond ((eq? tag 'applicative) (actual-value (car x) env))
          ((eq? tag 'lazy) (delay-it (car x) env))
...
(define (eval-definition exp env)
  (let ((def-val (definition-value exp)))
    (define-variable! (definition-variable exp)
      (cons (eval (car def-val) env) (cdr def-val))
      env)))
...
(define (definition-value exp)
  (if (symbol? (cadr exp))
      (caddr exp)
      (cons (make-lambda (map free (cdadr exp)) (cddr exp))
            (map eval-tag (cdadr exp)))))
